...
from codeqg.question_gen.bart.dataset import BARTCommentGenerationDataset
...
def generate(model, predict_dataset, tokenizer):
    print('predicting...')
    ...
    test_out = f'./inference/cg_bart_test_out-{test_datetime}.jsonl'
    out_data = []
    ...
    for data in tqdm.tqdm(chunked_dataset):
        ...
        for d in data:
            ...
            inputs = tokenizer(code, max_length=128, truncation=True, padding='longest', return_tensors='pt')
...
def train(args: argparse.Namespace):
    ...
    train_dataset = BARTCommentGenerationDataset('/home/zxw/inlinegeneration/bart/dataset/train.jsonl', tokenizer, tokenizer)
    ...
    valid_dataset = BARTCommentGenerationDataset('/home/zxw/inlinegeneration/bart/dataset/valid.jsonl', tokenizer, tokenizer)
    ...
    test_dataset = BARTCommentGenerationDataset('/home/zxw/inlinegeneration/bart/dataset/test.jsonl', tokenizer, tokenizer)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test_batch', '-teb', type=int, help='test batch size of model', required=False, default=32)
    parser.add_argument('--max_epochs', '-me', type=int, help='training max epoch of model', required=False, default=50)

    args = parser.parse_args()
    train(args)

# TODO: model parameter, machine hardware